from funkcje import wartosc, wzor
    ...
    if wartosc(wybor, poczatek) * wartosc(wybor, koniec) >= 0:
        ...
        if iteracje == 0: # d - osiągnięcie zadanej dokładności obliczeń
            ...
            for n in range(iteracje):
                x0 = (poczatek + koniec) / 2
                if wartosc(wybor, x0) == 0:
                    print("Korzystajac z metody bisekcji udalo sie znalezc dokladne rozwiazanie (epsilon = 0), po "
                          + str(n+1) + " iteracjach")
                    return x0
                if wartosc(wybor, x0) * wartosc(wybor, koniec) < 0:
                    poczatek = x0
...
def siecznych(wybor, poczatek, koniec, iteracje, eps):
    ...
    if wartosc(wybor, poczatek) * wartosc(wybor, koniec) >= 0:
        ...
        if iteracje == 0: # d - osiągnięcie zadanej dokładności obliczeń
            n = 1
            while abs(koniec - poczatek) >= eps:
                x0 = koniec - (wartosc(wybor, koniec) * (koniec - poczatek)) / (wartosc(wybor, koniec) - wartosc(wybor, poczatek))
                if wartosc(wybor, x0) == 0:
                    print("Korzystajac z metody siecznych udalo sie znalezc dokladne rozwiazanie (epsilon = 0), po "
                          + str(n+1) + " iteracjach")
                    return x0
                if wartosc(wybor, x0) * wartosc(wybor, koniec) < 0:
                    poczatek = x0
